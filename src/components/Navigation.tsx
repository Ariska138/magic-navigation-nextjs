      ...
      <ul className="flex relative">
        ...
        {Menus.map((menu, i) => (
          <li key={i} className="w-16">
            ...
            >
              <span
                className={`text-xl self-center z-10 cursor-pointer duration-500 ${
                  i === active && '-mt-6 text-white'
                }`}
              >
                {/* tidak bisa ganti warna */}
                {/* <Image
                  src={PersonIcon}
                  alt={''}
                  className={`${i === active && ' text-white'}`}
                  style={{ color: i === active ? 'white' : 'black' }}
                /> */}
                {/* bisa gerjalan dengan baik */}
                <UserIcon
                  className={`h-5 w-5`}
                  style={{
                    fill: i === active ? 'white' : 'black',
                    transition: 'fill 0.5s ease',
                  }}
                />
                {/* tidak bisa melakukan transisi */}
